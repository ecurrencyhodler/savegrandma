...
function hasMeaningfulChanges() {
  ...
  return unifiedStats.session.emailsScanned > initialStatsState.emailsScanned || unifiedStats.session.threatsIdentified > initialStatsState.threatsIdentified || unifiedStats.persistent.totalThreatsEverFound > initialStatsState.totalThreatsEverFound || unifiedStats.persistent.emailsWhitelisted > initialStatsState.emailsWhitelisted || unifiedStats.persistent.totalEmailsScanned > initialStatsState.totalEmailsScanned || emailWhitelist.size > initialStatsState.whitelistSize || hasDataChanged;
}
...
const SaveGrandmaDebug = {
  ...
  generateScanReport: async function () {
    ...
    if (this.scanSummary.scanStartTime && this.scanSummary.scanEndTime) {
      ...
      if (scanDuration > 3600) {
        // More than 1 hour seems suspicious
...
function getGmailAccountId(url = null) {
  ...
  const patterns = [/mail\.google\.com\/mail\/u\/(\d+)(?:\/|#|\?|$)/,
  // Standard pattern with query params
  /inbox\.google\.com\/u\/(\d+)(?:\/|#|\?|$)/,
  // Inbox pattern
...
async function flushPendingUpdates() {
  ...
  const hasUpdates = pendingUpdates.emailsScanned > 0 || pendingUpdates.threatsIdentified > 0 || pendingUpdates.emailsWhitelisted > 0;
  if (!hasUpdates) {
...
async function saveWhitelist() {
  return await executeWithLock(async () => {
    try {
      ...
      if (initialStatsState && emailWhitelist.size === initialStatsState.whitelistSize && initialWhitelistHash === currentWhitelistHash && !hasDataChanged) {
...
function checkDisplayNameMismatch(displayName, senderEmail, replyToEmail) {
  ...
  const hasFuzzyNameMatch = (displayName, emailPrefix) => {
    ...
    for (const namePart of nameParts) {
      ...
      if (namePart.length >= 3 && prefix.length >= 3 && namePart[0] === prefix[0] && Math.abs(namePart.length - prefix.length) <= 3) {
        return true;
      }
  ...
  if (replyToEmail && replyToEmail !== senderEmail) {
    ...
    const hasReplyToExactMatch = displayLower.includes(replyToFront) || displayLower.includes(replyToDomain) || replyToCompanyName && displayLower.includes(replyToCompanyName) || normalizedReplyToCompany && normalizedDisplay.includes(normalizedReplyToCompany) || normalizedReplyToCompany && normalizedReplyToCompany.includes(normalizedDisplay) || normalizedReplyToFront && normalizedDisplay.includes(normalizedReplyToFront) || normalizedReplyToFront && normalizedReplyToFront.includes(normalizedDisplay) || hasSignificantOverlap(normalizedDisplay, normalizedReplyToCompany) || hasSignificantOverlap(normalizedDisplay, normalizedReplyToFront);
...
function processEmailRow(element) {
  try {
    ...
    if (emailData.sender || emailData.subject) {
      ...
      const alreadyProcessed = SaveGrandmaDebug.emailRows.find(existing => existing.data.threadId && existing.data.threadId === emailData.threadId);
      if (!alreadyProcessed) {
...
function processOpenedEmail() {
  try {
    ...
    if (emailData.senderName || emailData.senderEmail || emailData.subject) {
      ...
      const alreadyInRows = SaveGrandmaDebug.emailRows.find(row => row.data.threadId && row.data.threadId === emailData.threadId);
      const alreadyOpened = SaveGrandmaDebug.openedEmails.find(opened => opened.data.threadId && opened.data.threadId === emailData.threadId);
...
function initializeDOMMonitoring() {
  ...
  try {
    ...
    window.SaveGrandmaDebug.testRaceConditionFix = function () {
      ...
      const operations = [() => saveStatsUnified(), () => saveWhitelist(), () => flushPendingUpdates()];
    ...
    window.SaveGrandmaDebug.validateStats = function () {
      ...
      const now = Date.now();
      if (unifiedStats.persistent.lastUpdated > now + 60000) {
...
async function initializeExtension() {
  try {
    ...
    console.error = function (...args) {
      ...
      if (message.includes('Cache storage prefetch failed') || message.includes('meetsw.js') || message.includes('Content Security Policy')) {
