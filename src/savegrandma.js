...
const SaveGrandmaDebug = {
  ...
  generateScanReport: function() {
    ...
    const scanDuration = this.scanSummary.scanEndTime ? 
      (this.scanSummary.scanEndTime - this.scanSummary.scanStartTime) / 1000 : 0;
...
function loadStats() {
  try {
    ...
    if (stored) {
      ...
      extensionStats = { ...extensionStats, ...parsedStats };
...
function checkDisplayNameMismatch(displayName, senderEmail, replyToEmail) {
  ...
  const hasMatch = displayLower.includes(senderFront) || 
  ...
  if (replyToEmail && replyToEmail !== senderEmail) {
    ...
    // Check for matches in both directions
    const hasReplyToMatch = displayLower.includes(replyToFront) || 
