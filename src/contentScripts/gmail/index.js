...
function showPhishingPopup(analysis, emailData) {
  ...
  if (isWhitelistFull) {
    markSafeButton.textContent = 'Whitelist Full (10,000 emails)';
    markSafeButton.disabled = true;
    markSafeButton.style.backgroundColor = '#6c757d';
  } else {
    markSafeButton.textContent = 'Mark as Safe (Whitelist Sender)';
    markSafeButton.onclick = async () => {
      try {
        markSafeButton.disabled = true;
        markSafeButton.textContent = 'Adding...';
        
        const result = await addEmailToWhitelist(emailData.senderEmail);
        
        if (result) {
          markSafeButton.textContent = 'Added to Whitelist!';
          markSafeButton.style.backgroundColor = '#28a745';
          
          // Remove warning icon from this email
          const emailRows = findElements(EMAIL_ROW_SELECTORS);
          emailRows.forEach(row => {
            const rowEmailData = extractEmailData(row);
            if (rowEmailData.threadId === emailData.threadId) {
